...
export default function InspectionPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (jRes.data.success) {
          ...
          if (jc.inspection.notes) {
            setGeneralNotes(jc.inspection.notes);
          }
        }
      } catch (err) {
  ...
  const handleFileChange = (
    ...
    if (files && files.length > 0) {
      ...
      setChecklist((prev) => ({
        ...prev,
        [id]: { ...prev[id], photos: [...prev[id].photos, newPhoto] },
      }));
    }
  };
    ...
    return (
      <div className="p-4">
        <div className="bg-red-50 border-2 border-red-200 rounded-xl p-6 text-center space-y-4 max-w-lg mx-auto">
          ...
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-white border-2 border-red-200 rounded-lg font-bold text-xs text-red-600 cursor-pointer"
          >
            Go Back
          </button>
        </div>
    ...
    <div className="w-full max-w-full mx-auto p-1 space-y-6 pb-20">
      ...
      >
        <div
          ...
          style={{ borderColor: themeColors.border }}
        >
          <div className="p-2 rounded-lg bg-red-50 text-red-500 border-2 border-red-100">
            <Bike size={20} />
          </div>
          <div>
            <h3
      ...
      <div className="space-y-4">
        ...
        {INSPECTION_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="p-4 rounded-xl border-2 space-y-4 transition-all bg-white"
              style={{ borderColor: themeColors.border }}
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  <div className="p-1.5 rounded-lg bg-gray-50 text-gray-400 border">
                    {item.icon}
                  </div>
              ...
              {state.status === "DAMAGED" && (
                <div
                  className="space-y-4 pt-4 border-t animate-in fade-in slide-in-from-top-2 duration-300"
                  style={{ borderColor: themeColors.border }}
                >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="w-16 h-16 rounded-lg border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300 hover:border-red-200 hover:text-red-400 transition cursor-pointer"
                      >
                        <Plus size={16} />
      ...
      >
        ...
        <textarea
          ...
          className="w-full px-3 py-2 rounded-lg border-2 text-[11px] min-h-[80px] transition focus:outline-none focus:border-red-400 bg-gray-50/30"
          style={{ borderColor: themeColors.border }}
          value={generalNotes}
      ...
      <div className="flex flex-col items-center gap-4 pt-6">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="w-full max-w-full py-3 rounded-xl text-white font-bold text-sm transition hover:opacity-95 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
